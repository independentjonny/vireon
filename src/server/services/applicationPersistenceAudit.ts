...
export const API_PERSISTENCE_MATRIX: ApiPersistenceMatrixRow[] = [
  ...
  { route: "/api/private-beta/financial-data-reset", status: "POSTGRES_BACKED", authoritativeState: true, target: "Preview-only reset_current_user_financial_data function plus retained audit_events receipt", phase2Action: "Deletes only the authenticated user's financial-domain records after exact confirmation. Account, identity, onboarding, preferences, feedback and audit history are retained; Production invocation is blocked." },
...
export const DOMAIN_PERSISTENCE_AUDIT: DomainPersistenceAuditRow[] = [
  ...
  },
  {
    ...
    activeApis: ["/api/runtime/*", "/api/build-*", "/api/private-beta/export", "/api/private-beta/deletion", "/api/private-beta/financial-data-reset", "/api/telemetry", "/api/logs/ingest"],
    activeUi: ["/autonomous-operations", "/production-readiness", "/privacy", "SupervisorInbox"],
    currentAuthority: "PostgreSQL for private-beta export/deletion lifecycle, Preview-only authenticated financial reset and product jobs; .ai/operations, .ai/builds and local telemetry remain operational tooling",
    targetAuthority: "background_jobs, data_exports, account_deletion_requests and audit_events where product-scoped",
    schemaAvailable: true,
    schemaGap: "0009 adds private-beta job/export/deletion lifecycle metadata. 0011 adds the Preview-only authenticated financial-domain reset function. General operational tooling remains excluded.",
    migrationRequired: true,
    localPathToRetire: [".ai/operations/* for product jobs", ".ai/builds/* for product jobs", ".ai/local-data/telemetry.json"],
    transactionBoundaries: "Export/deletion requests enqueue background jobs and audit events in one PostgreSQL transaction. Financial reset deletes only the authenticated user's financial domain and writes its retained audit receipt in one transaction.",
    ...
    stageStatus: "CORE_DOMAIN_CONVERTED",
    blocker: "Full account deletion and private-beta activation remain human approval gates. Preview financial-only reset is separately approved and Production-blocked.",
  },
];
