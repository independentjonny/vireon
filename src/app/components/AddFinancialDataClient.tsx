...
const verificationItems: Record<CategoryId, string[]> = {
  bank: ["Account ownership", "Current balances", "Income and spending", "Recurring payments"],
  employment: ["Employer", "Pay frequency", "Gross and net income", "Statement period"],
  property: ["Ownership and address", "Current value", "Rental income, if applicable", "Related loan details"],
  loans: ["Lender and account", "Outstanding balance", "Interest rate", "Repayment details"],
  tax: ["Taxpayer and period", "Taxable income", "Assessment outcome", "ATO source details"],
  super: ["Fund and member", "Current balance", "Contributions", "Statement period"],
  other: ["Document owner", "Document type", "Key financial values", "Source period"],
};
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] space-y-7 pb-10">
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_280px]">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_14px_40px_rgba(15,23,42,0.04)] sm:p-6">
            ...
            <div className="mt-5 flex flex-col gap-4 rounded-xl border border-blue-200 bg-blue-50/60 p-4 sm:flex-row sm:items-center">
              ...
              {summary.categoryStatuses[summary.recommendedCategory] === "Needs review"
                ? <Link href="/financial-vault/imports" className="inline-flex min-h-11 items-center justify-center rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-blue-800">{summary.recommendationAction}</Link>
      ...
      <footer className="flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center">
        ...
        {step === 1 ? selected.status === "Needs review" ? <Link href="/financial-vault/imports" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-blue-800">Review {selected.title.toLowerCase()}<ArrowRight className="h-4 w-4" /></Link> : <button type="button" onClick={() => setStep(2)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-blue-800">Continue<ArrowRight className="h-4 w-4" /></button> : step === 2 ? propertyFlow ? <button type="button" onClick={() => setStep(3)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-blue-800">{propertyPresentation.primaryAction}<ArrowRight className="h-4 w-4" /></button> : draft.selectedDocuments.length > 0 ? <Link href={`/financial-vault/imports?documentId=${encodeURIComponent(draft.selectedDocuments[0].id)}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-blue-800">{selected.status === "Confirmed" ? "View verified document" : "Review & verify"} ({draft.selectedDocuments.length})<ArrowRight className="h-4 w-4" /></Link> : <button type="button" disabled className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-300 px-5 text-sm font-semibold text-white" title="Choose or upload at least one document first">Review &amp; verify<ArrowRight className="h-4 w-4" /></button> : propertyState === "confirmed" ? <Link href="/financial-profile" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-blue-800">Back to Financial Position<ArrowRight className="h-4 w-4" /></Link> : <button type="button" onClick={() => void confirmProperty()} disabled={submitBusy} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-700 px-5 text-sm font-semibold text-white hover:bg-blue-800 disabled:bg-slate-400">{submitBusy ? "Saving…" : editingExisting ? "Confirm & update Financial Position" : "Confirm & save to Financial Position"}<ArrowRight className="h-4 w-4" /></button>}
      </footer>
    </div>
  );
}
