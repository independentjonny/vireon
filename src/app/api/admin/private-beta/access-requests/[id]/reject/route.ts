...
export function createRejectAccessRequestHandler(deps?: Partial<PrivateBetaInvitationServiceDeps>, authorize: AdminAuth = defaultAdminAuth()) {
  return async function POST(request: Request, context: { params: Promise<{ id: string }> } | { params: { id: string } }) {
    const auth = await authorize(request);
    if (!auth.ok) return auth.response;
    const params = await context.params;
    const body = await readJsonBody(request);
    const result = await accessService(deps).rejectAccessRequest({
...
export const POST = createRejectAccessRequestHandler();
